...
void terms::eval_additive_aux(const model& m, const atom_index& i, const atom_index& j, fl r, flv& out) const { // out is added to
    ...
    fl theta;
    vec random = vec(-11.183, -19.157, -34.745);
/*************************************************************************************************************/
    ...
    if (xs_hal_any_bond_possible(a.xs, b.xs)) {     //if there is a halogen bond possible, we need the angle
        if (xs_is_halogen(a.xs)) {                  //a is the halogen, b is the acceptor
            vec a_coords = m.coords[i.i];           //coords of halogen
            ...
            vec b_coords = m.coords[i.i];           //coords of halogen
            ...
            VINA_FOR_IN(j, bonds) {
                ...
                if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                    vec C_coords = m.coords[bs.connected_atom_index.i];     //coords of carbon
                    vec v1 = b_coords - C_coords;  //vector between halogen and carbon
                    vec v2 = b_coords - a.coords;  //vector between halogen and acceptor
    ...
    if (xs_sul_bond_possible(a.xs, b.xs)) {     //if there is a halogen bond possible, we need the angle
        if (xs_is_S(a.xs)) {                    //a is the sulfur, b is the acceptor
            ...
            if (bonds.size() == 2) {          //sulfur must be bonded to exactly two carbons
                flv theta_holder;
                vec a_coords = m.coords[i.i];           //coords of sulfur
                VINA_FOR_IN(j, bonds) {
                    const bond& bs = bonds[j];
                    const atom& c = m.get_atom(bs.connected_atom_index);
                    if(c.ad == AD_TYPE_A){              //the sulfur needs to be connected to an aromatic carbon
                        vec C_coords = m.coords[bs.connected_atom_index.i];     //coords of carbon
                        vec v1 = a_coords - C_coords;  //vector between sulfur and carbon
                        vec v2 = a_coords - b.coords;  //vector between sulfur and acceptor
...
/*************************************************************************************************************/
	...
	VINA_FOR_IN(k, usable_terms)
        if(r < usable_terms[k].cutoff){
			...
			out[offset + k] += usable_terms[k].eval(a, b, r, theta_rounded);
        }
