...
struct gauss : public usable {
    ...
    fl eval(sz t1, sz t2, fl r, fl theta) const {
...
struct halogen_cl_bond : public usable {
	fl good_1;
	fl good_2;
    fl bad;
    halogen_cl_bond(fl good_1_, fl good_2_, fl bad_, fl cutoff_) : usable(cutoff_), good_1(good_1_), good_2(good_2_), bad(bad_) {
        name = std::string("halogen_cl_bond(g=") + to_string(good_1) + ", b=" + to_string(bad) + ")";
    }
	...
	//fl eval(sz t1, sz t2, fl r) const {
        ...
        fl ClB = -0.016;
        fl Clv = 2.46;
        fl ClAngle = (ClA*cos(((180-theta)*Clv)*pi/180.0)+ClB) / (ClA+ClB); //max is 1
        ClAngle = ClAngle < 0 ? 0 : ClAngle;
        ClAngle = theta < 140 ? 0 : ClAngle;
        if(xs_hal_cl_bond_possible(t1, t2))
            return slope_step(bad, good, r - optimal_distance(t1, t2)) * ClAngle;
        return 0;
	}
 };
 
struct halogen_br_bond : public usable {
	fl good_1;
	fl good_2;
    fl good_3;
    fl bad;
    halogen_br_bond(fl good_1_, fl good_2_, fl good_3_, fl bad_, fl cutoff_) : usable(cutoff_), good_1(good_1_), good_2(good_2_), good_3(good_3_), bad(bad_) {
        name = std::string("halogen_br_bond(g=") + to_string(good_1) + ", b=" + to_string(bad) + ")";
    }
    ...
    fl eval(sz t1, sz t2, fl r, fl theta) const {
        ...
        fl BrB = 0.15;
        fl Brv = 2.42;
        fl BrAngle = (BrA*cos(((180-theta)*Brv)*pi/180.0)+BrB) / (BrA+BrB); //max is 1
        BrAngle = BrAngle < 0 ? 0 : BrAngle;
        BrAngle = theta < 120 ? 0 : BrAngle;
        if(xs_hal_br_bond_possible(t1, t2))
            return slope_step(bad, good, r - optimal_distance(t1, t2)) * BrAngle;
        return 0;
	}
 };
 
struct halogen_i_bond : public usable {
	fl good_1;
	fl good_2;
    fl good_3;
    fl bad;
    halogen_i_bond(fl good_1_, fl good_2_, fl good_3_, fl bad_, fl cutoff_) : usable(cutoff_), good_1(good_1_), good_2(good_2_), good_3(good_3_), bad(bad_) {
        name = std::string("halogen_i_bond(g=") + to_string(good_1) + ", b=" + to_string(bad) + ")";
    }
    ...
    fl eval(sz t1, sz t2, fl r, fl theta) const {
        ...
        fl IB = 0.29;
        fl Iv = 2.23;
        fl IAngle = (IA*cos(((180-theta)*Iv)*pi/180.0)+IB) / (IA+IB); //max is 1
        IAngle = IAngle < 0 ? 0 : IAngle;
        IAngle = theta < 120 ? 0 : IAngle;
        if(xs_hal_i_bond_possible(t1, t2)) {
            return slope_step(bad, good, r - optimal_distance(t1, t2)) * IAngle;
        }
        return 0;
	}
 };

struct sulfur_bond : public usable {
    fl good;
    fl good_theta;
    fl bad;
    fl bad_theta;
    sulfur_bond(fl good_, fl good_theta_, fl bad_, fl bad_theta_, fl cutoff_) : usable(cutoff_), good(good_), good_theta(good_theta_), bad(bad_), bad_theta(bad_theta_) {
        name = std::string("sulfur_bond(g=") + to_string(good) + ", b=" + to_string(bad) + ")";
    }
    
    fl eval(sz t1, sz t2, fl r, fl theta) const {
        if(xs_sul_bond_possible(t1, t2)) {
            return slope_step(bad, good, r - optimal_distance(t1, t2)) * slope_step(bad_theta, good_theta, theta);
        }
        return 0;
    }
};
/*
struct halogen_cl_angle : public angle_term {
    fl good_1;
    fl good_2;
    fl bad;
    halogen_cl_angle(fl good_1_, fl good_2_, fl bad_, fl cutoff_) : angle_term(cutoff_), good_1(good_1_), good_2(good_2_), bad(bad_) {
        name = std::string("halogen_cl_angle(g=") + to_string(good_1) + ", b=" + to_string(bad) + ")";
    }
...
struct halogen_br_angle : public angle_term {
    fl good_1;
    fl good_2;
    fl good_3;
    fl bad;
    halogen_br_angle(fl good_1_, fl good_2_, fl good_3_, fl bad_, fl cutoff_) : angle_term(cutoff_), good_1(good_1_), good_2(good_2_), good_3(good_3_), bad(bad_) {
        name = std::string("halogen_br_angle(g=") + to_string(good_1) + ", b=" + to_string(bad) + ")";
    }
...
struct halogen_i_angle : public angle_term {
    fl good_1;
    fl good_2;
    fl good_3;
    fl bad;
    halogen_i_angle(fl good_1_, fl good_2_, fl good_3_, fl bad_, fl cutoff_) : angle_term(cutoff_), good_1(good_1_), good_2(good_2_), good_3(good_3_), bad(bad_) {
        name = std::string("halogen_i_angle(g=") + to_string(good_1) + ", b=" + to_string(bad) + ")";
    }
...
struct sulfur_angle : public angle_term {
    fl good;
    fl good_theta;
    fl bad;
    fl bad_theta;
    sulfur_angle(fl good_, fl good_theta_, fl bad_, fl bad_theta_, fl cutoff_) : angle_term(cutoff_), good(good_), good_theta(good_theta_), bad(bad_), bad_theta(bad_theta_) {
        name = std::string("sulfur_angle(g=") + to_string(good) + ", b=" + to_string(bad) + ")";
    }
...
/*****************************************************/
    ...
    add(1, new halogen_cl_bond(-0.25, -0.15, 0,cutoff));
    
    add(1, new halogen_br_bond(-0.45, -0.35, -0.25, 0,cutoff));
    
    add(1, new halogen_i_bond(-0.55, -0.45, -0.35, 0,cutoff));
    
    add(1, new sulfur_bond(-0.2, 165, 0, 140,cutoff));
    /*
    add(1, new halogen_cl_angle(-0.25, -0.15, 0,cutoff));
    
    add(1, new halogen_br_angle(-0.45, -0.35, -0.25, 0,cutoff));
    
    add(1, new halogen_i_angle(-0.55, -0.45, -0.35, 0,cutoff));
    
    add(1, new sulfur_angle(-0.2, 165, 0, 140,cutoff));
    */
/******************************************************/
