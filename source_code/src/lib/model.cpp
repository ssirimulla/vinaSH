...
fl eval_interacting_pairs(const precalculate& p, fl v, const interacting_pairs& pairs, const vecv& coords, const atomv& atoms) { // clean up
	...
	VINA_FOR_IN(i, pairs) {
        ...
        if (xs_hal_any_bond_possible(a.xs, b.xs)) {     //if there is a halogen bond possible, we need the angle
            if (xs_is_halogen(a.xs)) {                  //a is the halogen, b is the acceptor
                ...
                VINA_FOR_IN(j, bonds) {
                    ...
                    const atom& c = atoms[bs.connected_atom_index.i];
                    if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                        vec C_coords = coords[bs.connected_atom_index.i];     //coords of carbon
                        vec v1 = coords[ip.a] - C_coords;  //vector between halogen and carbon
                        vec v2 = coords[ip.a] - b.coords;  //vector between halogen and acceptor
                        theta = angle(v1, v2);
                    }
                }
            }
                ...
                VINA_FOR_IN(j, bonds) {
                    ...
                    if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                        ...
                        vec v2 = b.coords - coords[ip.a];  //vector between halogen and acceptor
                ...
                if (bonds.size() == 2) {          //sulfur must be bonded to exactly two carbons
                    ...
                    VINA_FOR_IN(j, bonds) {
                        ...
                        if(c.ad == AD_TYPE_A){              //the sulfur needs to be connected to an aromatic carbon
                            vec C_coords = coords[bs.connected_atom_index.i];     //coords of carbon
                            vec v1 = coords[ip.a] - C_coords;  //vector between sulfur and carbon
                            vec v2 = coords[ip.a] - b.coords;  //vector between sulfur and acceptor
		...
		if(r2 < cutoff_sqr) {
            fl tmp = p.eval_fast(ip.type_pair_index, r2, theta);
			curl(tmp, v);
			e += tmp;
		}
	}
...
fl eval_interacting_pairs_deriv(const precalculate& p, fl v, const interacting_pairs& pairs, const vecv& coords, vecv& forces, const atomv& atoms) {  // clean up
	const fl cutoff_sqr = p.cutoff_sqr();
	fl e = 0;
    fl theta;
	VINA_FOR_IN(i, pairs) {
		...
		fl r2 = sqr(r);
		if(r2 < cutoff_sqr) {
            pr tmp = p.eval_deriv(ip.type_pair_index, r2, theta);
...
fl model::evali(const precalculate& p,                                  const vec& v                          ) const { // clean up
	fl e = 0;
	VINA_FOR_IN(i, ligands)
        e += eval_interacting_pairs(p, v[0], ligands[i].pairs, internal_coords, atoms); // probably might was well use coords here
	return e;
}

fl model::evale(const precalculate& p, const igrid& ig, const vec& v                          ) const { // clean up
	fl e = ig.eval(*this, v[1]);
	e += eval_interacting_pairs(p, v[2], other_pairs, coords, atoms);
	return e;
}
...
fl model::eval_deriv  (const precalculate& p, const igrid& ig, const vec& v, const conf& c, change& g) { // clean up
	...
	e += eval_interacting_pairs_deriv(p, v[2], other_pairs, coords, minus_forces, atoms);  // adds to minus_forces
	VINA_FOR_IN(i, ligands)
		e += eval_interacting_pairs_deriv(p, v[0], ligands[i].pairs, coords, minus_forces, atoms);  // adds to minus_forces
...
fl model::eval_intramolecular(const precalculate& p, const vec& v, const conf& c) {
		...
		VINA_FOR_IN(j, grid_atoms) {
            ...
            if (xs_hal_any_bond_possible(a.xs, b.xs)) {     //if there is a halogen bond possible, we need the angle
                if (xs_is_halogen(a.xs)) {                  //a is the halogen, b is the acceptor
                    ...
                    VINA_FOR_IN(j, bonds) {
                        const bond& bs = bonds[j];
                        const atom& c = get_atom(bs.connected_atom_index);
                        if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                            vec C_coords = coords[bs.connected_atom_index.i];     //coords of carbon
                            vec v1 = coords[i] - C_coords;  //vector between halogen and carbon
                            vec v2 = coords[i] - b.coords;  //vector between halogen and acceptor
                            theta = angle(v1, v2);
                        }
                    }
                }
                    ...
                    VINA_FOR_IN(j, bonds) {
                        ...
                        if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                            vec v1 = b.coords - c.coords;  //vector between halogen and carbon
                            vec v2 = b.coords - coords[i];  //vector between halogen and acceptor
                            fl theta = angle(v1, v2);
                        }
                    }
                }
                ...
                if (xs_is_S(a.xs)) {                    //a is the sulfur, b is the acceptor
                    ...
                    if (bonds.size() == 2) {          //sulfur must be bonded to exactly two carbons
                        ...
                        VINA_FOR_IN(j, bonds) {
                            ...
                            if(c.ad == AD_TYPE_A){              //the sulfur needs to be connected to an aromatic carbon
                                vec C_coords = coords[bs.connected_atom_index.i];     //coords of carbon
                                vec v1 = coords[i] - C_coords;  //vector between sulfur and carbon
                                vec v2 = coords[i] - b.coords;  //vector between sulfur and acceptor
                                theta_holder.push_back(angle(v1, v2));
                            }
                            else {
                                theta_holder.push_back(0);         //if the sulfur is bonded to a non-aromatic carbon, the angle is 0
			...
			if(r2 < cutoff_sqr) {
                ...
                fl this_e = p.eval_fast(type_pair_index, r2, theta);
				curl(this_e, v[1]);
				e += this_e;
			}
		}
	...
	VINA_FOR_IN(i, other_pairs) {
        ...
        fl theta;
        const atom& a = get_atom(sz_to_atom_index(pair.a));
        const atom& b = get_atom(sz_to_atom_index(pair.b));
        if (xs_hal_any_bond_possible(a.xs, b.xs)) {     //if there is a halogen bond possible, we need the angle
            if (xs_is_halogen(a.xs)) {                  //a is the halogen, b is the acceptor
                const std::vector<bond>& bonds = a.bonds;
                VINA_FOR_IN(j, bonds) {
                    ...
                    if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                        vec C_coords = coords[bs.connected_atom_index.i];     //coords of carbon
                        vec v1 = coords[pair.a] - C_coords;  //vector between halogen and carbon
                        vec v2 = coords[pair.a] - b.coords;  //vector between halogen and acceptor
                        theta = angle(v1, v2);
                    }
                }
            }
                ...
                VINA_FOR_IN(j, bonds) {
                    ...
                    if(c.el == EL_TYPE_C){              //the halogen needs to be connected to a carbon
                        vec v1 = b.coords - c.coords;  //vector between halogen and carbon
                        vec v2 = b.coords - coords[pair.a];  //vector between halogen and acceptor
                        fl theta = angle(v1, v2);
                    }
                }
            }
            ...
            if (xs_is_S(a.xs)) {                    //a is the sulfur, b is the acceptor
                ...
                if (bonds.size() == 2) {          //sulfur must be bonded to exactly two carbons
                    ...
                    VINA_FOR_IN(j, bonds) {
                        ...
                        if(c.ad == AD_TYPE_A){              //the sulfur needs to be connected to an aromatic carbon
                            vec C_coords = coords[bs.connected_atom_index.i];     //coords of carbon
                            vec v1 = coords[pair.a] - C_coords;  //vector between sulfur and carbon
                            vec v2 = coords[pair.a] - b.coords;  //vector between sulfur and acceptor
                            theta_holder.push_back(angle(v1, v2));
                        }
                        else {
                            theta_holder.push_back(0);         //if the sulfur is bonded to a non-aromatic carbon, the angle is 0
		...
		if(r2 < cutoff_sqr) {
            fl this_e = p.eval_fast(pair.type_pair_index, r2, theta);
			curl(this_e, v[2]);
			e += this_e;
		}
	}
